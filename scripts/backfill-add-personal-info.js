...
/**
 * Backfill: populate personalInfo for existing users
 * Sets personalInfo = { lastName: null, firstName: null, middleInitial: null, phoneNumber: null }
 * Only for users who have missing or empty personalInfo.
 *
 * Usage:
 *   DRY RUN:
 *     $env:MONGO_URI="mongodb://..." ; node .\scripts\backfill-add-personal-info.js --dry-run
...
async function run() {
  ...
  try {
    ...
    console.log(`Users needing backfill: ${count}`);
    ...
    const result = await User.updateMany(
      ...
      {
        runValidators: false, // IMPORTANT: required fields would fail otherwise
      }
    );

    console.log("Backfill completed:", result);
    console.log(`Updated ${result.modifiedCount || result.nModified} users.`);
  } catch (err) {
