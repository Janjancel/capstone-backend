/**
 * backfill-orders-quezon.js
 *
 * Usage (PowerShell):
 *   $env:MONGO_URI="your_mongo_connection_string"; node backfill-orders-quezon.js
 *
 * Notes:
 * - Creates 1 order per day for the past 6 months (based on current date).
 * - Order model + Counter included so orderId generation matches your schema.
 * - Addresses are generated within Quezon province (Region IV-A).
 */
...
// Quezon municipalities / cities (Region IV-A)
const QUEZON_CITIES = [
  "Lucena City",
  "Tayabas",
  "Lucban",
...
async function main() {
  ...
  for (let d = new Date(startDate); d <= endDate; d.setDate(d.getDate() + 1)) {
    ...
    for (let i = 0; i < itemCount; i++) {
      ...
      items.push({
        id: `item-${Date.now().toString(36)}-${i}-${randInt(0,9999)}`, // unique-ish id
        name,
        quantity,
        price,
        subtotal,
    ...
    // Generate realistic address within Quezon province (Region IV-A)
    const city = pick(QUEZON_CITIES);
    const postal = String(randInt(4300, 4325));
    // Quezon roughly: lat 13.7--14.4, lng 121.0--122.0 (approximate bounding box)
    const lat = Number((13.7 + Math.random() * (14.4 - 13.7)).toFixed(6));
    const lng = Number((121.0 + Math.random() * (122.0 - 121.0)).toFixed(6));
    ...
    const orderDoc = new Order({
      ...
      address: {
        ...
        barangay: `Brgy. ${pick(["Poblacion","Centro","Santo Niño","San Roque","San Isidro","Mabini","Santa Cruz"])}`,
        city,
        province: "Quezon",
    ...
    try {
      // run validate so pre('validate') hook sets orderId properly
