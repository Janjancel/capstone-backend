// scripts/backfill-orderId.js
/* eslint-disable no-console */
const mongoose = require("mongoose");

const { Schema, model } = mongoose;

// ---- Counter model (monthly atomic sequence) ----
const counterSchema = new Schema({
  key: { type: String, unique: true, index: true }, // e.g., 'order:10-25'
  seq: { type: Number, default: 0 },
});
const Counter = mongoose.models.Counter || model("Counter", counterSchema);

// ---- Lightweight Order model (bypass validations) ----
// Keep collection name exactly "orders" (default for Order model)
const OrderLite = mongoose.models.OrderLite || model(
  "OrderLite",
  new Schema({}, { strict: false, collection: "orders" })
);

function formatOrderId(date, seq) {
  const mm = String(date.getMonth() + 1).padStart(2, "0");
  const yy = String(date.getFullYear() % 100).padStart(2, "0");
  const seqStr = String(seq).padStart(4, "0");
  return `${mm}-O-${seqStr}-${yy}`;
}

async function nextMonthlySeq(date) {
  const mm = String(date.getMonth() + 1).padStart(2, "0");
  const yy = String(date.getFullYear() % 100).padStart(2, "0");
  const key = `order:${mm}-${yy}`;
  const doc = await Counter.findOneAndUpdate(
    { key },
    { $inc: { seq: 1 } },
    { new: true, upsert: true, setDefaultsOnInsert: true }
  );
  return doc.seq;
}

function getBaseDate(doc) {
  if (doc.createdAt) return new Date(doc.createdAt);
  if (doc._id && typeof doc._id.getTimestamp === "function") return doc._id.getTimestamp();
  try {
    const oid = new mongoose.Types.ObjectId(doc._id);
    return oid.getTimestamp();
  } catch {
    return new Date();
  }
}

async function ensurePartialUniqueIndex() {
  const coll = mongoose.connection.collection("orders");
  // Unique only for docs that actually HAVE orderId
  await coll.createIndex(
    { orderId: 1 },
    { unique: true, partialFilterExpression: { orderId: { $exists: true } } }
  );
  console.log("✅ Ensured partial unique index on orders.orderId");
}

async function backfill() {
const query = {
  $or: [
    { orderId: { $exists: false } }, // field not present
    { orderId: null },               // explicit null
    { orderId: { $regex: /^\s*$/ } } // empty or whitespace string
  ],
};

  const cursor = OrderLite.find(query).cursor();
  let scanned = 0;
  let updated = 0;

  for await (const o of cursor) {
    scanned += 1;

    const baseDate = getBaseDate(o);

    let attempts = 0;
    while (attempts < 4) {
      attempts += 1;
      try {
        const seq = await nextMonthlySeq(baseDate);
        const orderId = formatOrderId(baseDate, seq);
        await OrderLite.updateOne({ _id: o._id }, { $set: { orderId } });
        updated += 1;
        break;
      } catch (e) {
        if (e && e.code === 11000) {
          console.warn(`Duplicate orderId for ${o._id}, retrying…`);
          continue; // grab a new seq and retry
        }
        console.error(`❌ Failed for ${o._id}:`, e);
        break;
      }
    }
  }

  console.log(`Backfill complete. Scanned: ${scanned}, Updated: ${updated}`);
}

(async () => {
  const uri = process.env.MONGO_URI;
  if (!uri) {
    console.error("Please set MONGO_URI in your environment.");
    process.exit(1);
  }

  await mongoose.connect(uri, { autoIndex: false });
  console.log("Connected to MongoDB");

  await ensurePartialUniqueIndex();
  await backfill();

  await mongoose.disconnect();
  console.log("Disconnected");
})();
