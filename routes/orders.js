 ...
 *  - userId (required)
 *  - items: [{ id, quantity, price, name, image, images, subtotal }, ...]
 *  - address: object (free-form)
 *  - notes: optional
 *  - deliveryFee: number (frontend computed; server will validate and persist)
 *
 * Behavior:
 *  - Uploads any attached files (multipart) to Cloudinary (if present).
 *  - Normalizes items and computes authoritative items subtotal server-side.
 *  - Accepts deliveryFee from payload (if provided and a valid non-negative number).
 *  - Writes deliveryFee to order, and writes total = itemsSubtotal + deliveryFee.
 */
router.post("/", upload.array("images"), async (req, res) => {
  try {
    // --- 0) Validate userId presence early
    const userId = req.body.userId || (req.body && req.body.user && req.body.user.userId);
    if (!userId) {
      return res.status(400).json({ error: "Missing userId" });
    }

    // 1) Upload any incoming files to Cloudinary (multipart uploads)
    let uploadedItems = [];
    if (req.files && req.files.length > 0) {
      uploadedItems = await Promise.all(
        req.files.map(
    ...
    // 4) Compute items subtotal safely (server-source-of-truth)
    const itemsSubtotal = items.reduce((sum, i) => sum + (Number(i.subtotal) || 0), 0);
    ...
    // 6) Delivery fee handling: accept from frontend but validate type & non-negativity.
    // If frontend didn't send deliveryFee, default to 0.
    let providedDeliveryFee = 0;
    if (req.body.deliveryFee !== undefined && req.body.deliveryFee !== null && req.body.deliveryFee !== "") {
      // in multipart forms, numbers may come as strings
      const parsed = Number(req.body.deliveryFee);
      if (Number.isFinite(parsed) && parsed >= 0) {
        providedDeliveryFee = parsed;
      } else {
        // invalid delivery fee => reject
        return res.status(400).json({ error: "Invalid deliveryFee provided" });
      }
    }

    // 7) Build order object and compute authoritative total (items subtotal + delivery fee)
    const computedTotal = +(itemsSubtotal + providedDeliveryFee).toFixed(2);

    const newOrder = new Order({
      userId,
      items,
      // store itemsSubtotal separately if schema allows; otherwise store the authoritative total in 'total'
      itemsSubtotal: +itemsSubtotal.toFixed(2),
      deliveryFee: +providedDeliveryFee.toFixed(2),
      total: computedTotal,
      address,
      notes,
    });
    ...
    // Return the created order (full object) so frontend can read the saved deliveryFee/total
    return res.status(201).json(newOrder);
  } catch (error) {
    console.error("Order creation error:", error);
