  ...
  try {
    ...
    if (!user) {
      ...
      try {
        ...
        // The userId will be automatically generated by the pre-save middleware
        await user.save();
        console.log("New Google user created successfully with ID:", user.userId);
      } catch (saveError) {
    ...
    res.status(200).json({
      ...
      user: {
        _id: user._id,
        userId: user.userId, // Include the new userId field
        email: user.email,
        username: user.username,
        role: user.role,
        status: user.status,
