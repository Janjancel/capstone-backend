...
// POST a new request with multiple images
router.post(
  "/",
  upload.fields([
  ...
  async (req, res) => {
    try {
      const { userId, name, contact, price, description, location, notes } =
        req.body;
      ...
      if (
        ...
        return res
          .status(400)
          .json({ error: "All three images (front, back, side) are required" });
      }

      const frontUrl = await uploadImage(req.files.front[0].buffer);
      const backUrl = await uploadImage(req.files.back[0].buffer);
      const sideUrl = await uploadImage(req.files.side[0].buffer);
      ...
      res
        .status(201)
        .json({ message: "Request submitted successfully", data: saved });
    } catch (err) {
      console.error("❌ Server error:", err);
      res.status(500).json({ error: err.message || "Server error" });
    }
...
router.patch(
  ...
  async (req, res) => {
    try {
      ...
      if (req.files.front) updateData["images.front"] = await uploadImage(req.files.front[0].buffer);
      if (req.files.back) updateData["images.back"] = await uploadImage(req.files.back[0].buffer);
      if (req.files.side) updateData["images.side"] = await uploadImage(req.files.side[0].buffer);
