...
// /**
//  * NOTE (IMPORTANT):
//  * The deleteReview helper below is intentionally permissive: it deletes by id
//  * without checking requester identity or admin role. This matches your request
//  * "must allow all user to delete any review".
//  *
//  * WARNING: This is insecure in production. Replace with proper auth/authorization:
//  * - verify JWT (req.user)
//  * - check req.user.role or ownership before deleting
//  */

// // Helper: delete a review by id (permissive)
...
// // Delete a single review for a specific user (backwards-compatible route).
// // **BEHAVIOR CHANGE**: This route now deletes the review by id regardless of userId param.
// router.delete("/users/:userId/reviews/:reviewId", async (req, res) => {
//   try {
//     const { reviewId } = req.params;
//     const deleted = await deleteReview({ reviewId });
//     if (!deleted) return res.status(404).json({ message: "Review not found" });
//     return res.json({ message: "Review deleted", review: deleted });
//   } catch (err) {
//     console.error("Failed to delete user review:", err);
//     res.status(500).json({ message: "Failed to delete review" });
//   }
...
// // Delete all reviews for a user (kept as-is)
// router.delete("/users/:userId/reviews", async (req, res) => {
//   try {
//     await Review.deleteMany({ userId: req.params.userId });
//     res.json({ message: "All reviews cleared for user" });
 ...
 * The delete helpers and endpoints below intentionally allow deletion of any review by id
 * (no ownership or admin checks). This matches your request "must allow all user to delete any review".
 *
 * This is insecure for production. Replace with proper auth/authorization as soon as possible:
 *  - verify JWT and set req.user
 *  - check req.user.role or ownership before allowing deletion
 */
...
// Backwards-compatible route (was owner-only) — now permissive: deletes by reviewId regardless of :userId
router.delete("/users/:userId/reviews/:reviewId", async (req, res) => {
  try {
    const { reviewId } = req.params;
...
// Generic delete by id (permissive)
router.delete("/:id", async (req, res) => {
  try {
    const reviewId = req.params.id;
    const deleted = await deleteReviewById(reviewId);
    ...
    // Fire-and-forget deletion notification (best-effort)
    (async () => {
      try {
        const notifPayload = {
          userId: deleted.userId ? String(deleted.userId) : undefined,
          ...
          role: "admin",
          for: "sell",
          message: `A review (${deleted.rating}/5) was deleted.`,
        };
        const n = new Notification(notifPayload);
        await n.save();
      } catch (nErr) {
...
// Update a review (partial update) for a specific user — still enforces userId in query path
router.patch("/users/:userId/reviews/:reviewId", async (req, res) => {
  try {
    const updated = await Review.findOneAndUpdate(
      { _id: req.params.reviewId, userId: req.params.userId },
